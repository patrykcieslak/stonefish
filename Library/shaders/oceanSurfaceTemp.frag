...
void main()
{
	...
	//Sun contribution
    float sunSkyColor = (reflectedSunRadiance(sunDirection, toEye, normal, Tx, Ty, sigmaSq) * Isun * SunShadow(P)/whitePoint).r * 0.0001;
	
	//Sky and scene reflection
	vec3 ray = reflect(-toEye, normalize(vec3(normal.xy, -3.0)));
	vec3 trans;
	sunSkyColor += (GetSkyLuminance(Psky - center, ray, 0.0, sunDirection, trans) / whitePoint).r * 0.0001;
    
    //Temperature based on sky emissivity
	float cosTheta = clamp(dot(normalize(eyePos - center), -toEye), 0.001, 1.0);
	float emissivity = 1.0 - pow(1.0 - skyEmissivity, 0.555556/cosTheta); 
	float T = airTemperature + 273.15;
	sunSkyColor += pow(emissivity * T*T*T*T, 0.25) - 273.15;

	//Sea contribution
    fragColor = mix(waterTemperature, sunSkyColor, fresnel);
}
