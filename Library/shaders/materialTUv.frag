...
uniform bool enableAlbedoTex;
uniform bool enableNormalTex;
uniform bool enableTemperatureTex;
...
{	
	...
	if(enableAlbedoTex)
	{
		vec4 texColor = texture(texAlbedo, texCoord);
		absorption = vec4(clamp(1.0 - mix(color.rgb, texColor.rgb, color.a), 1.0, 0.001), 1.0);
	}
	if(enableNormalTex)
	{
	...
	float temperature = temperatureRange.x;
	if(enableTemperatureTex)
	{
		temperature = texture(texTemperature, texCoord).r;
		temperature = temperatureRange.x + (temperatureRange.y - temperatureRange.x) * temperature;
	}
