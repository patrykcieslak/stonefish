...
{
    vec4 input1 = textureLod(texSource, vec3(texcoord.x, i.x, layer), 0.0);
    vec4 input2 = textureLod(texSource, vec3(texcoord.x, i.y, layer), 0.0);
...
void main() 
{
	vec4 data = textureLod(texButterfly, vec2(texcoord.y, pass), 0.0);
	vec2 i = data.xy;
    vec2 w = data.zw;
    fragColor = fft2(gl_Layer, i, w);
}
