...
{
    vec4 input1 = textureLod(texSource, vec3(i.x, texcoord.y, layer), 0.0);
    vec4 input2 = textureLod(texSource, vec3(i.y, texcoord.y, layer), 0.0);
...
void main() 
{
    vec4 data = textureLod(texButterfly, vec2(texcoord.x, pass), 0.0);
    vec2 i = data.xy;
    vec2 w = data.zw;
