...
OpenGLOcean::OpenGLOcean(float geometricWaves, SDL_mutex* hydrodynamics)
{
    ...
    OpenGLState::BindTexture(TEX_BASE, GL_TEXTURE_2D, oceanTextures[0]);
    ...
    OpenGLState::BindTexture(TEX_BASE, GL_TEXTURE_2D, oceanTextures[1]);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    ...
    OpenGLState::BindTexture(TEX_BASE, GL_TEXTURE_2D, oceanTextures[5]);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    ...
    OpenGLState::BindTexture(TEX_BASE, GL_TEXTURE_3D, oceanTextures[2]);
    ...
    OpenGLState::BindTexture(TEX_BASE, GL_TEXTURE_2D_ARRAY, oceanTextures[3]);
    ...
    OpenGLState::BindTexture(TEX_BASE, GL_TEXTURE_2D_ARRAY, oceanTextures[4]);
    glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, GL_RGBA32F, params.fftSize, params.fftSize, layers, 0, GL_RGBA, GL_FLOAT, NULL);
    glTexParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
    glTexParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_WRAP_S, GL_REPEAT);
    ...
    {
        shader = new GLSLShader(precompiled, header, "oceanSurface.frag", "oceanSurface.vert");
    ...
    oceanShaders[0]->Use();
    oceanShaders[0]->SetUniform("transmittance_texture", TEX_ATM_TRANSMITTANCE);
    oceanShaders[0]->SetUniform("scattering_texture", TEX_ATM_SCATTERING);
    oceanShaders[0]->SetUniform("irradiance_texture", TEX_ATM_IRRADIANCE);
    ...
    {
        shader = new GLSLShader(precompiled, header, "oceanBacksurface.frag", "oceanSurface.vert");
        shader->AddUniform("size", ParameterType::FLOAT);
    }
    ...
    oceanShaders[1]->Use();
    oceanShaders[1]->SetUniform("transmittance_texture", TEX_ATM_TRANSMITTANCE);
    oceanShaders[1]->SetUniform("scattering_texture", TEX_ATM_SCATTERING);
    oceanShaders[1]->SetUniform("irradiance_texture", TEX_ATM_IRRADIANCE);
    ...
    //Background
    shader = new GLSLShader(precompiled, header, "oceanBackground.frag", "oceanSurface.vert");
    shader->AddUniform("MVP", ParameterType::MAT4);
    shader->AddUniform("FC", ParameterType::FLOAT);
    shader->AddUniform("size", ParameterType::FLOAT);
    shader->AddUniform("eyePos", ParameterType::VEC3);
    ...
    oceanShaders[11]->Use();
    oceanShaders[11]->SetUniform("transmittance_texture", TEX_ATM_TRANSMITTANCE);
    oceanShaders[11]->SetUniform("scattering_texture", TEX_ATM_SCATTERING);
    oceanShaders[11]->SetUniform("irradiance_texture", TEX_ATM_IRRADIANCE);    
...
{	
    ...
    if(waves)
    {
        ...
        {
            OpenGLState::BindTexture(TEX_POSTPROCESS1, GL_TEXTURE_2D_ARRAY, oceanTextures[4]);
...
void OpenGLOcean::DrawUnderwaterMask(OpenGLCamera* cam)
{
    ...
    if(waves)
    {
        ...
        oceanShaders[7]->SetUniform("FC", cam->GetLogDepthConstant());
        ...
        oceanShaders[8]->Use();
        oceanShaders[8]->SetUniform("MVP", cam->GetProjectionMatrix() * cam->GetViewMatrix());
        oceanShaders[8]->SetUniform("FC", cam->GetLogDepthConstant());
        oceanShaders[8]->SetUniform("size", oceanSize);
    ...
    else
    {
        ...
        oceanShaders[7]->SetUniform("MVP", cam->GetProjectionMatrix() * cam->GetViewMatrix());
        oceanShaders[7]->SetUniform("FC", cam->GetLogDepthConstant());
        oceanShaders[7]->SetUniform("size", oceanSize);
...
{
    oceanShaders[11]->Use();
    oceanShaders[11]->SetUniform("MVP", cam->GetProjectionMatrix() * cam->GetViewMatrix());
    oceanShaders[11]->SetUniform("FC", cam->GetLogDepthConstant());
    oceanShaders[11]->SetUniform("size", oceanSize);
    oceanShaders[11]->SetUniform("eyePos", cam->GetEyePosition());
    oceanShaders[11]->SetUniform("cWater", getLightAttenuation());
    oceanShaders[11]->SetUniform("bWater", getLightScattering());
...
void OpenGLOcean::DrawSurface(OpenGLCamera* cam)
{
    ...
    if(waves)
    {
        ...
        oceanShaders[0]->SetUniform("FC", cam->GetLogDepthConstant());
    ...
    else //Flat surface (infinite plane)
    {
        ...
        oceanShaders[0]->SetUniform("MV", glm::mat3(glm::transpose(glm::inverse(cam->GetViewMatrix()))));
        oceanShaders[0]->SetUniform("FC", cam->GetLogDepthConstant());
        oceanShaders[0]->SetUniform("size", oceanSize);
        oceanShaders[0]->SetUniform("viewport", glm::vec2((GLfloat)viewport[2], (GLfloat)viewport[3]));
        oceanShaders[0]->SetUniform("eyePos", cam->GetEyePosition());
        oceanShaders[0]->SetUniform("gridSizes", params.gridSizes);
        oceanShaders[0]->SetUniform("texWaveFFT", TEX_POSTPROCESS1);
...
void OpenGLOcean::DrawBacksurface(OpenGLCamera* cam)
{
    ...
    if(waves)
    {
        ...
        oceanShaders[1]->SetUniform("FC", cam->GetLogDepthConstant());
        ...
        oceanShaders[1]->SetUniform("cWater", getLightAttenuation());
        oceanShaders[1]->SetUniform("bWater", getLightScattering());
    ...
    else
    {
        ...
        oceanShaders[1]->SetUniform("MV", glm::mat3(glm::transpose(glm::inverse(cam->GetViewMatrix()))));
        oceanShaders[1]->SetUniform("FC", cam->GetLogDepthConstant());
        oceanShaders[1]->SetUniform("size", oceanSize);
        oceanShaders[1]->SetUniform("eyePos", cam->GetEyePosition());
        oceanShaders[1]->SetUniform("gridSizes", params.gridSizes);
        oceanShaders[1]->SetUniform("cWater", getLightAttenuation());
        oceanShaders[1]->SetUniform("bWater", getLightScattering());
...
void OpenGLOcean::DrawVolume(OpenGLCamera* cam, GLuint sceneTexture, GLuint linearDepthTex)
{
    ...
    oceanShaders[9]->Use();
    oceanShaders[9]->SetUniform("cWater", getLightAttenuation());
    oceanShaders[9]->SetUniform("bWater", getLightScattering());
