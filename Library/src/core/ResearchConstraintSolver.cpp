...
ResearchConstraintSolver::ResearchConstraintSolver(btMLCPSolverInterface* mlcp) : btMultiBodyConstraintSolver()
{
    ...
    m_fallback = 0;
    m_cfm = 0.0;
    m_interleaveContactAndFriction = false;
    m_gUseMatrixMultiply = false;
}

ResearchConstraintSolver::~ResearchConstraintSolver()
{
    delete m_solver;
}

int ResearchConstraintSolver::getNumFallbacks() const
{
    return m_fallback;
}
...
bool ResearchConstraintSolver::solveMLCP(const btContactSolverInfo& infoGlobal)
{
	bool result = true;
...
void ResearchConstraintSolver::createMLCPFast(const btContactSolverInfo& infoGlobal)
{
	int numContactRows = m_interleaveContactAndFriction ? 3 : 1;
...
void ResearchConstraintSolver::createMLCP(const btContactSolverInfo& infoGlobal)
{
...
Scalar ResearchConstraintSolver::solveGroupCacheFriendlySetup(btCollisionObject** bodies, int numBodiesUnUsed, btPersistentManifold** manifoldPtr, int numManifolds,btTypedConstraint** constraints,int numConstraints,const btContactSolverInfo& infoGlobal,btIDebugDraw* debugDrawer)
{
    ...
    
	{
		...
		if (m_interleaveContactAndFriction)
		{
