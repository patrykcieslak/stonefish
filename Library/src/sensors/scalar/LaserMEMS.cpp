...
LaserMEMS::LaserMEMS(std::string uniqueName, unsigned int num_lines, unsigned int num_points, Scalar fov_horizontal, Scalar fov_vertical, Scalar frequency, int historyLength) : LinkSensor(uniqueName, frequency, historyLength)
{
    ...
    for(unsigned int i=0; i <= numL*numP; ++i)
    {
    ...
    distances = std::vector<Scalar>(2000+1, Scalar(0));
}
...
void LaserMEMS::InternalUpdate(Scalar dt)
{
    ...
    for (unsigned int i = 0; i < numP; ++i){
        for (unsigned int j = 0; j < numL; ++j){
             ...
             Scalar currentVerticalAngle = -0.25 + j * verticalStep; // Adjust the starting angle
        // Calculate direction based on both horizontal and vertical angles
        ...
        if(closest.hasHit())
        {
            ...
            distances[j+numL*i] = (p - mbTrans.getOrigin()).length();
        }
        else
            distances[j+numL*i] = channels[i].rangeMax;
     }
    }
    
    //record sample
    Sample s(numP*numL+1, distances.data());
        ...
        for (unsigned int i = 0; i < numP; ++i)
        {
            for (unsigned int j = 0; j < numL; ++j)
            {
                ...
                item.points.push_back(glm::vec3(
                    dir.x() *  distances[j+numL*i],
                    dir.y() *  distances[j+numL*i],
                    dir.z() *  distances[j+numL*i]
                ));
            }
        }
