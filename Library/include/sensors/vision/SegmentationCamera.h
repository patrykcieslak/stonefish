...
namespace sf
{
    ...
    public:
        ...
        SegmentationCamera(const std::string& uniqueName, unsigned int resolutionX, unsigned int resolutionY, Scalar hFOVDeg, Scalar frequency = Scalar(-1), 
            Scalar near = Scalar(STD_NEAR_PLANE_DISTANCE), Scalar far = Scalar(STD_FAR_PLANE_DISTANCE)); //Rendering options
        ...
        */
        static std::unique_ptr<SegmentationCamera> Construct(const std::string& uniqueName, Scalar frequency, ConstructInfo& info);
